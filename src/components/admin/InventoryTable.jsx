...
const InventoryTable = ({
  ...
  const filteredBooks = useMemo(() => {
    if (!debouncedSearch) return displayBooks;
    const query = debouncedSearch.toLowerCase();
    return displayBooks.filter(
      (book) =>
        book.title.toLowerCase().includes(query) ||
        book.author.toLowerCase().includes(query) ||
        book.category.toLowerCase().includes(query)
    );
  }, [debouncedSearch, displayBooks]);
  ...
  const handleDelete = useCallback(
    (id) => {
      ...
      setTimeout(() => {
        onDeleteBook?.(id);
        // Keep it removed even after API call
      }, 500);
    },
    [onDeleteBook]
  );
    ...
    <div className="space-y-4">
      ...
      {isLoading && (
        ...
        >
          ...
          >
            <div className="w-8 h-8 border-4 border-gray-200 border-t-red-600 rounded-full" />
          </motion.div>
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <p className="text-sm text-gray-600">
              ...
              {debouncedSearch && (
                <>
                  {" "}
                  of <span className="font-semibold">{displayBooks.length}</span> total
                </>
              )}
            </p>
          </div>
